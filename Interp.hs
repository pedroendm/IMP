...
updateState (x, y) [] = [(x,y)] -- Care
updateState (x, y) ((x',y'):xs) = if x == x' then (x,y) : xs
                                else (x',y') : updateState (x,y) xs
...
interpCom (Comp com1 com2) s    = let s' = interpCom com1 s
                                  in interpCom com2 s'
interpCom (If bexp com1 com2) s = case interpBExp bexp s of
                                    True -> interpCom com1 s
                                    False -> interpCom com2 s
interpCom (While bexp com) s    = interpCom (If bexp (Comp com (While bexp com)) Skip) s

interpAExp :: AExp -> State -> Int
interpAExp (Num n)       s = n
interpAExp (Var id)      s = lookupState id s
interpAExp (Plus a1 a2)  s = let
                                a1' = interpAExp a1 s
                                a2' = interpAExp a2 s
                             in
                                a1' + a2'
interpAExp (Minus a1 a2) s = let
                                a1' = interpAExp a1 s
                                a2' = interpAExp a2 s
                              in
                               a1' - a2'
interpAExp (Times a1 a2) s = let
                                a1' = interpAExp a1 s
                                a2' = interpAExp a2 s
                              in
                                a1' * a2'
...
interpBExp (T t) s = t
interpBExp (Eq a1 a2) s = let
                            a1' = interpAExp a1 s
                            a2' = interpAExp a2 s
                          in
                            a1' == a2'
interpBExp (Leq a1 a2) s = let
                            a1' = interpAExp a1 s
                            a2' = interpAExp a2 s
                          in
                            a1' <= a2'
